...
import { faTimes, faBoxOpen, faUser, faInfoCircle, faTruck, faFileLines } from "@fortawesome/free-solid-svg-icons";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col">
        ...
        {/* Header - Fixed */}
        <div className="flex justify-between items-center bg-gray-100 px-6 py-4 rounded-t-xl border-b">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-xl font-bold text-gray-800">Order #{order.orderId}</h3>
              <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${order.status === 'Completed' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                {order.status}
              </span>
            </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-y-4 gap-x-6 bg-gray-50 p-4 rounded-lg border">
              ...
              <div>
                ...
                <p className="text-gray-800">{displayVal(order.contactNumber)}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase">FB Name / Email</p>
                <p className="text-gray-800 text-sm">{order.fbName || order.email ? `${order.fbName} / ${order.email}` : "-"}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase">Date of Birth</p>
                <p className="text-gray-800">{order.dateOfBirth ? new Date(order.dateOfBirth).toLocaleDateString() : "-"}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase">Sales Person</p>
                <p className="text-gray-800 font-medium">{displayVal(order.salesPerson)}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase">Order Type / Channel</p>
                <p className="text-gray-800">{order.orderType} via {order.channel}</p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gray-50 p-4 rounded-lg border">
                ...
                <p className="text-gray-800 text-sm whitespace-pre-wrap">{displayVal(order.address)}</p>
                <p className="text-gray-800 text-sm mt-1">{order.postCode} {order.state}</p>
              </div>
                ...
                <tbody>
                  {order.orderPackages?.map((pkg, pIdx) => (
                    <React.Fragment key={pIdx}>
                      ...
                      {/* Show products inside this package */}
